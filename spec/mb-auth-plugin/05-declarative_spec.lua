...
local crypto = require "kong.plugins.mb-auth-pligin.crypto"
local cjson   = require "cjson"


for _, strategy in helpers.each_strategy() do
  describe("mb-auth-pligin declarative config #" .. strategy, function()
    local db
    lazy_setup(function()
      local _
...
--      assert(helpers.stop_kong())
    ...
    local mbbasicauth_credential_def = {
    ...
    local mbbasicauth_hashed_credential_def = {
    ...
    before_each(function()
      ...
      db.mbbasicauth_credentials:truncate()
      ...
      assert(declarative.load_into_db({
        ...
        mbbasicauth_credentials = { [mbbasicauth_credential_def.id] = mbbasicauth_credential_def },
      }, { _transform = true }))

      assert(declarative.load_into_db({
        mbbasicauth_credentials = { [mbbasicauth_hashed_credential_def.id] = mbbasicauth_hashed_credential_def },
      }, { _transform = false }))
    end)
    ...
    describe("load_into_db", function()
      it("imports base and custom entities with associations", function()
        ...
        assert.equals("mb-auth-pligin", plugin.name)
        assert.same(plugin_def.config, plugin.config)

        local mbbasicauth_credential = assert(db.mbbasicauth_credentials:select({ id = mbbasicauth_credential_def.id }))
        assert.equals(mbbasicauth_credential_def.id, mbbasicauth_credential.id)
        assert.equals(consumer.id, mbbasicauth_credential.consumer.id)
        assert.equals("james", mbbasicauth_credential.username)
        assert.equals(crypto.hash(consumer.id, "secret"), mbbasicauth_credential.password)

        local mbbasicauth_hashed_credential = assert(db.mbbasicauth_credentials:select({ id = mbbasicauth_hashed_credential_def.id }))
        assert.equals(mbbasicauth_hashed_credential_def.id, mbbasicauth_hashed_credential.id)
        assert.equals(consumer.id, mbbasicauth_hashed_credential.consumer.id)
        assert.equals("bond", mbbasicauth_hashed_credential.username)
        assert.equals(mbbasicauth_hashed_credential_def.password, mbbasicauth_hashed_credential.password)
      end)
    end)
      ...
      describe("Authorized", function()
        it("Accepts valid credentials", function()
          local creds = "Basic " .. ngx.encode_base64(
                          string.format("%s:%s", mbbasicauth_credential_def.username,
                                                 mbbasicauth_credential_def.password))
          ...
          assert.equal(mbbasicauth_credential_def.username, json.headers["x-credential-identifier"])
          assert.equal(mbbasicauth_credential_def.username, json.headers["x-credential-username"])
        end)

        it("Accepts valid credentials (introduced with a hashed password)", function()
          local creds = "Basic " .. ngx.encode_base64(
                          string.format("%s:%s", mbbasicauth_hashed_credential_def.username,
                                                 "MI6"))
          ...
          assert.equal(consumer_def.username, json.headers["x-consumer-username"])
          assert.equal(consumer_def.custom_id, json.headers["x-consumer-custom-id"])
          assert.equal(mbbasicauth_hashed_credential_def.username, json.headers["x-credential-identifier"])
          assert.equal(mbbasicauth_hashed_credential_def.username, json.headers["x-credential-username"])
        end)
      end)
    end)
  end)
