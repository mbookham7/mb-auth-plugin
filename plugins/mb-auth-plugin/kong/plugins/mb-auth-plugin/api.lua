...
local kong               = kong
local credentials_schema = kong.db.mbbasicauth_credentials.schema
local consumers_schema   = kong.db.consumers.schema

return {
  ["/consumers/:consumers/mb-auth-plugin/"] = {
  ...
  ["/consumers/:consumers/mb-auth-plugin/:mbbasicauth_credentials"] = {
    schema = credentials_schema,
    methods = {
      before = function(self, db)
        ...
        if self.req.method ~= "PUT" then
          ...
          self.params.mbbasicauth_credentials = cred.id
        end
      end,
