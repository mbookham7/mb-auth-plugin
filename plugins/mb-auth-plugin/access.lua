...
local function load_credential_into_memory(username)
  local credential, err = kong.db.mbbasicauth_credentials:select_by_username(username)
  if err then
    return nil, err
  end
  return credential
...
local function load_credential_from_db(username)
  ...
  local credential_cache_key = kong.db.mbbasicauth_credentials:cache_key(username)
